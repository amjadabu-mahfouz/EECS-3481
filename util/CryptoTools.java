...
{8.12,1.49,2.71,4.32,12.02,2.3,2.03,5.92,7.31,0.1,0.69,3.98,2.61,6.95,7.68,1.82,0.11,6.02,6.28,9.1,2.88,1.11,2.09,0.17,2.11,0.07};
	...
	public static double getIC(byte[] ar)
	{
		...
		for (int i = 1; i <= trials; i++)
		{
			...
			if (ar[pos1] == ar[pos2]) ic++;
			//System.out.println(ic / (double) i);
		}		
		
		
		return ic / (double) trials;
	}
	...
	public static String xor(String a, String b) {
		...
		for(int i = 0; i < a.length(); i++) {
			...
			int k = Integer.parseInt(b.charAt(i)+"");
			res += (j + k)%2; 
		}
	...
	public static String bitwise(String og) {
		String res = "";
	...
	public static String bin2Hex(String bin) {
		...
		for(int i = 0; i < bin.length(); i += 4) {
			...
			int t1 = Integer.parseInt(temp, 2);
			String hex = Integer.toHexString(t1);
			res += hex.toUpperCase();
